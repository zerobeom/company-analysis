...
- **질적 펀더멘탈** — 날짜/평가(Good·Bad·Neutral)/내용/코멘트로 새 항목을
  추가합니다. 각 항목의 ✎ 버튼으로 수정, ✕ 버튼으로 삭제할 수 있습니다.
- **새 기업 추가** — 목록 페이지 상단의 "+ 새 기업 추가"로 기업명/티커/국가/
  투자 이유를 입력하면 새 기업 페이지가 자동 생성됩니다. slug(내부 식별
  코드)는 티커를 소문자로 변환해 자동으로 만들어지므로 따로 입력하지
  않아도 됩니다.
